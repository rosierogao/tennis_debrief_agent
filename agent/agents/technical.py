"""
Technical agent wrapper: returns technical_hypotheses JSON.
...
class TechnicalAgent:
    """Produces structured technical hypotheses."""

    def __init__(self) -> None:
        self.prompt_path = Path(__file__).parent.parent / "prompts" / "technical.md"
    ...
    def _validate_output(self, obj: Dict[str, Any]) -> None:
        require_keys(obj, ["technical_hypotheses", "confidence"])
        items = require_list_of_dict(obj.get("technical_hypotheses"), "technical_hypotheses", 4)
        for item in items:
