"""
Mental agent wrapper: returns mental_observations JSON.
...
class MentalAgent:
    """Produces structured mental observations."""

    def __init__(self) -> None:
        self.prompt_path = Path(__file__).parent.parent / "prompts" / "mental.md"
    ...
    def _validate_output(self, obj: Dict[str, Any]) -> None:
        require_keys(obj, ["mental_observations", "confidence"])
        items = require_list_of_dict(obj.get("mental_observations"), "mental_observations", 4)
        for item in items:
