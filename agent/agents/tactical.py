"""
Tactical agent wrapper: returns tactical_observations JSON.
...
class TacticalAgent:
    """Produces structured tactical observations."""

    def __init__(self) -> None:
        self.prompt_path = Path(__file__).parent.parent / "prompts" / "tactical.md"
    ...
    def _validate_output(self, obj: Dict[str, Any]) -> None:
        require_keys(obj, ["tactical_observations", "confidence"])
        items = require_list_of_dict(obj.get("tactical_observations"), "tactical_observations", 4)
        for item in items:
